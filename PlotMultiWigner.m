function [] = PlotMultiWigner( mat , x , hamfunc , contouryes , values )
%PLOTMULTIWIGNER Plots Multiple Wigner-Ville distributions of columnvectors in mat
...
if(contouryes==1)
    %contourf(XX/2,-YY/2,abs(Wf)*100000000,'LineStyle','none');
    imagesc(x/2,-y/2,abs(Wf));
    hold on, 
        contour(XX/2,YY/2,abs(norm1(hamfunc(XX/2,YY/2),inf)),values,'k'), 
    hold off;
else
    imagesc(x/2,-y/2,abs(Wf)*10000000);
    hold on, 
        contour(XX/2,YY/2,hamfunc(XX/2,YY/2),values,'k'), 
    hold off;
end;
box on;
